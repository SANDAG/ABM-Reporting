...
class AccessMeasures(object):
    ...
    def scenario_path(self) -> str:
        ...
        with self.conn.connect() as conn:
            path = pd.read_sql(
                sql="SELECT RTRIM([path]) AS [path] FROM [abm_14_2_0].[dimension].[scenario] WHERE [scenario_id] =  " +
                    str(self.scenario_id),
                con=conn)
    ...
    def calc_transit_access(self, criteria: str, max_time: int,
        ...
        # no longer needeed given we only have 1 walk skim in ABM2+
        ...
        skim_file = pd.read_csv(
            skim_path, 
            names=["i", "j", "time"], # headers not brought in
            usecols=["i",     # origin MGRA
                     "j",     # destination MGRA
                     "time"]) # time in minutes

        # filter out MGRA-MGRA transit access skims
        # based on input maximum skim time parameter
        if max_time == 30:
            skim_file = skim_file[skim_file["time"] <= 30]
        elif max_time == 45:
            skim_file = skim_file[skim_file["time"] <= 45]
        else:
            raise ValueError("Invalid parameter: max_time must be in {30,45}")
        ...
        accessible_origins = skim_file[skim_file["j"].isin(d)]
        accessible_origins = list(set(accessible_origins["i"]))

        # old method for ABM2
        # where the mgra-mgra walk skim file was in a wide format
        """"" 
