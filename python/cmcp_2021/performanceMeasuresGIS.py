        ...
        if not self.check_scenario():
            ...
            with settings.engines["ABM-Reporting"].begin() as conn:
                conn.execute(
                    ...
                    str(self.scenario_id) + " AND [measure] IN " +
                    "('Population Near High Frequency Transit', " +
                    "'Multifamily Housing Near High Frequency Transit', " +
                    "'Multifamily Population Near Transit')"
                )
